...
export const postVisibilityType = pgEnum('post_visibility_type', [
	'public',
	'private',
])
...
export const postVisibility = pgTable(
	...
	(table) => {
		return {
			...
			postVisibilityPkey: primaryKey({
				columns: [table.postId, table.userId],
				name: 'post_visibility_pkey',
			}),
		}
	},
)

export const wallPost = pgTable(
	'wall_post',
	{
		wallId: uuid('wall_id').notNull(),
		postId: uuid('post_id').notNull(),
	...
	(table) => {
		return {
			...
			wallPostPkey: primaryKey({
				columns: [table.wallId, table.postId],
				name: 'wall_post_pkey',
			}),
		}
	},
